...
import { getCoreApiBaseUrl } from "../../lib/apiClient";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${getCoreApiBaseUrl()}/api/v1/auth/login`, {
        method: "POST",
        headers: {
      ...
      // Store token and basic user info for later API calls
      if (typeof window !== "undefined") {
        localStorage.setItem("cc_access_token", json.data.token);
        localStorage.setItem("cc_user", JSON.stringify(json.data.user || {}));
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F2F4F7] px-4 py-8">
