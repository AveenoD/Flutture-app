...
export function Stepper({ steps, activeStep, className = "", onStepClick, completedSteps = [] }: StepperProps) {
  ...
  return (
    <div className={`relative mb-8 md:mb-10 ${className}`}>
      ...
      {/* Active Progress Line - fills up to active step, but only within the visible track
          (from left 12% to right 12%), so we shrink the computed width (0–100%)
          down to the actual track span (76%). */}
      <div
            ...
            >
              ...
              <p
                className={`text-sm md:text-base ${
                  isActive || isCompleted ? "font-semibold" : "font-medium"
                }`}
              >
                {step.label}
              </p>
