    ...
    <div className={`bg-white rounded-xl p-4 sm:p-5 lg:p-6 border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-shadow transition-colors ${className}`}>
      ...
      {showInput && (
        <div className="flex items-center gap-2 sm:gap-3 w-full">
          ...
          <button
            type="button"
            className={`w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-full bg-[var(--primary-base)] text-white flex items-center justify-center hover:bg-[var(--primary-hover)] transition-all flex-shrink-0 shadow-sm ${
              canSubmit ? "hover:scale-105" : ""
            }`}
            title={canSubmit ? "Send" : "Voice input"}
            onClick={() => {
              if (!canSubmit) return;
              onInputSubmit?.();
            }}
