...
import { MapPin, Buildings, Tag, User, Phone, Envelope, ShieldCheck, Star, Sparkle, Shield, Pencil, ShareNetwork, Eye } from "phosphor-react";
import { PriceBreakdownCard } from "./priceBreakdownCard";
import type { PriceBreakdownItem } from "./priceBreakdownCard";

export interface QuotationData {
  id: string;
  /** Set when row comes from API (GET /api/v1/quotations). */
  leadId?: string;
  status?: "approved" | "pending" | "draft"; // Quotation status
  project: {
    name: string;
    image: string;
    location: string;
  ...
  createdAt?: Date | string; // For sorting and filtering
}
...
export function QuotationCard({ quotation, onEdit, onShare, onView }: QuotationCardProps) {
  return (
      ...
      <div className="p-3 sm:p-4 border-b border-slate-200">
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div>
            ...
            <p className="text-xs font-semibold text-slate-900">{quotation.allocatedFlat.flatNo || "N/A"}</p>
          </div>
          <div>
            <p className="text-[10px] text-slate-500 mb-0.5">Floor</p>
            <p className="text-xs font-semibold text-slate-900">{quotation.allocatedFlat.floor || "N/A"}</p>
          </div>
        </div>
        <div>
          <p className="text-[10px] text-slate-500 mb-0.5">Customer</p>
          <p className="text-xs font-semibold text-slate-900">{quotation.clientInfo.customerName}</p>
        </div>
      </div>
