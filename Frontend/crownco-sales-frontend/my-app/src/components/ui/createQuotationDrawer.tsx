...
import { X, Buildings, MapPin, Tag, User, Phone, Envelope, ShieldCheck, Star, Sparkle, Shield, Plus, Trash } from "phosphor-react";
...
export function CreateQuotationDrawer({ isOpen, onClose, onCreateQuotation, onUpdateQuotation, editingQuotation, projects = [] }: CreateQuotationDrawerProps) {
  ...
  const [newPriceItem, setNewPriceItem] = useState({ label: "", amount: "", isDiscount: false });
                ...
                {formData.priceBreakdown.map((item, index) => (
                  <div key={index} className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                    <div className="flex-1">
                      <div className="text-sm font-medium text-[var(--sidebar-text-main)]">{item.label}</div>
