...
export default function FollowUpDetailPage() {
  ...
  const leadData: LeadData | null = useMemo(() => {
    ...
    return {
      id: 1, // UI-only; backend lead ids are UUIDs
      name: lead.name ?? "—",
      phone: lead.phone ?? "—",
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : "Failed to load follow-up detail.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6]">
      <div className="max-w-[1400px] xl:max-w-[1600px] 2xl:max-w-[1800px] mx-auto px-3 sm:px-4 md:px-6 lg:px-8 xl:px-10 2xl:px-12 py-4 sm:py-6 lg:py-8 xl:py-10">
        {/* Page Header */}
        <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-5 lg:mb-6 md:mb-8">
          <button
            onClick={() => router.back()}
          ...
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-[#2D3748]">Follow Ups Detail</h1>
        </div>
        ...
        {!loading && !error && leadData && callRecording && (
          <FollowUpPage leadData={leadData} remarks={remarks} callRecording={callRecording} />
        )}
      </div>
    </div>
  );
