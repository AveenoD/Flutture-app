  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] flex flex-col">
      <div className="max-w-[1400px] xl:max-w-[1600px] 2xl:max-w-[1800px] mx-auto w-full px-3 sm:px-4 md:px-6 lg:px-8 xl:px-10 2xl:px-12 py-4 sm:py-6 lg:py-8 flex flex-col flex-1">
        {/* Chat Header */}
        <header className="flex items-center pb-3 sm:pb-4 lg:pb-5 mb-3 sm:mb-4 lg:mb-5 border-b border-[var(--sidebar-border-color)] flex-shrink-0">
        ...
        {/* User Info Card */}
        <div className="bg-white border border-[var(--sidebar-border-color)] rounded-xl p-3 sm:p-4 md:p-5 lg:p-6 mb-3 sm:mb-4 lg:mb-6 shadow-sm hover:shadow-md transition-shadow duration-200 flex-shrink-0">
          <div className="flex items-center justify-between gap-2 sm:gap-3">
            <div className="flex items-center gap-2.5 sm:gap-3 md:gap-3.5 lg:gap-4 min-w-0 flex-1">
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 md:w-[50px] md:h-[50px] rounded-full overflow-hidden border-2 border-[var(--sidebar-border-color)] bg-[var(--surface-neutral)] flex-shrink-0">
                <Image
            ...
            <span className="px-2.5 sm:px-3 md:px-3.5 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-semibold uppercase tracking-wide whitespace-nowrap bg-[var(--surface-error)] text-[var(--error)] flex-shrink-0">
              Very Hot
            </span>
          </div>
        </div>
        ...
        <div
          ref={messageAreaRef}
          className="flex-1 overflow-y-auto overflow-x-hidden px-2 sm:px-3 md:px-4 py-3 sm:py-4 lg:py-5 flex flex-col gap-3 sm:gap-4 scroll-smooth chat-scrollbar min-h-0"
        >
        {messages.map((item) => {
          // Date Divider
          if ("date" in item) {
            return (
              ...
              >
                <div className="absolute left-0 right-0 top-1/2 h-px bg-[var(--sidebar-border-color)]"></div>
                <span className="relative bg-white px-3 sm:px-4 py-1 sm:py-1.5 rounded-xl text-[10px] sm:text-xs font-medium text-[var(--sidebar-text-sub)] border border-[var(--sidebar-border-color)]">
                  {item.date}
                </span>
              </div>
            );
          ...
          return (
            <div
              key={msg.id}
              className={`flex flex-col gap-1 max-w-[90%] sm:max-w-[85%] md:max-w-[75%] lg:max-w-[70%] xl:max-w-[65%] animate-[fadeInUp_0.3s_ease-out] ${
            ...
            >
              {msg.file ? (
                ...
                <div
                  ...
                  className="flex items-center gap-2 sm:gap-3 bg-[var(--sidebar-bg-hover)] border border-[var(--sidebar-border-color)] px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl cursor-pointer transition-all duration-200 hover:bg-[var(--surface-primary)] hover:border-[var(--primary-base)] hover:-translate-y-0.5 hover:shadow-md max-w-full"
                >
                ...
                // Text Message
                <div
                  className={`px-3 sm:px-4 py-2.5 sm:py-3 rounded-2xl text-xs sm:text-sm md:text-[0.9375rem] leading-relaxed shadow-sm break-words ${
                    msg.isOutgoing
                      ? "bg-[var(--primary-base)] text-white rounded-br-md hover:bg-[var(--primary-hover)]"
                      : "bg-[var(--sidebar-bg-hover)] text-[var(--foreground)] rounded-bl-md border border-[var(--sidebar-border-color)]"
                  }`}
                >
                  {msg.text}
                </div>
              ...
              {/* Message Info */}
              <div
                className={`flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs text-[var(--sidebar-text-muted)] px-1 ${
                  ...
                  <>
                    ...
                    {msg.isRead ? (
                      <Checks
                        size={12}
                        weight="fill"
                        className="text-white/80 sm:w-3.5 sm:h-3.5"
                      />
                    ) : (
                      <Check
                        size={12}
        ...
        {/* Input Container */}
        <div className="pt-3 sm:pt-4 lg:pt-5 border-t border-[var(--sidebar-border-color)] mt-2 sm:mt-3 lg:mt-4 flex-shrink-0">
          <div className="bg-white border border-[var(--sidebar-border-color)] rounded-3xl flex items-center px-2 sm:px-3 py-1.5 sm:py-2 gap-1.5 sm:gap-2 transition-all duration-200 focus-within:border-[var(--primary-base)] focus-within:shadow-[0_0_0_3px_var(--primary-selected)]">
            <button
